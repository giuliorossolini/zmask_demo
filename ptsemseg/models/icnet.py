...
class icnet(nn.Module):
    ...
    def __init__(
        ...
        self.max_pool_point = nn.Identity()
    ...
    def forward(self, x):
        ...
        x_sub2 = self.max_pool_point(x_sub2)
        ...
        if self.training:
            ...
            #---------------------------------------------------
            # @Giulio 
            if hasattr(self,'multiple_outputs'):
    ...
    def protected_forward(self, x, defense_module = None):
        h, w = x.shape[2:]
        ...
        # H/8, W/8 -> H/16, W/16
        x_sub2 = self.res_block2(x_sub2)
        self.trigger, self.mask = defense_module(x_sub2)


        #---------------------------------------------------
        #Improving the effect
        x = self.set_mask(self.mask, x)
        h, w = x.shape[2:]
        # H, W -> H/2, W/2
        x_sub2 = F.interpolate(
            x, size=get_interp_size(x, s_factor=2), mode="bilinear", align_corners=True
    ...
    def forward_and_get_activations(self, x):
        ...
        return [sp_1, sp_2, sp_3], [sp_1, sp_3, cnt_1], ['convbnrelu1_2', 'res_block2', 'res_block5'], ['convbnrelu1_2',  'res_block5', 'conv5_4_k1']


    def forward_features(self, data, label=None):
        #return self.forward_features_res_block2(data)
        return self.forward_features_res_block3(data)
    
    def forward_deep_features(self,data, label=None):
        return self.forward_features_res5(data)

    def forward_all_features(self, data, label=None):
        return self.forward_features_res5_res3(data)


    def forward_features_res5(self, x):
        h, w = x.shape[2:]
    ...
    def forward_features_res5_res3(self, x):
        h, w = x.shape[2:]
    ...
    def forward_features_res_block3(self, x):
        h, w = x.shape[2:]
    ...
    def forward_features_sub12(self, x):
        h, w = x.shape[2:]
